...
echo -n "Total repositories pushed to Harbor: "
curl -s -u ${HARBOR_USERNAME}:${HARBOR_PASSWORD} "http://${HARBOR_HOST}/v2/_catalog" | jq -r '.repositories[]' | wc -l
echo ""
echo "Test the registry:"
echo "  docker login ${REGISTRY_ENTRY} -u ${HARBOR_USERNAME}"
echo "  docker pull ${REGISTRY_ENTRY}/${HARBOR_PROJECT}/mesosphere/konvoy-image-builder:v2.22.3"
echo "  docker pull ${REGISTRY_ENTRY}/${HARBOR_PROJECT}/mesosphere/kommander:17.42.26"
echo ""
echo "Note: Images require specific version tags (not :latest)"
echo "To find available tags for an image:"
echo "  curl -s -u ${HARBOR_USERNAME}:${HARBOR_PASSWORD} http://${HARBOR_HOST}/v2/${HARBOR_PROJECT}/<image-path>/tags/list"
echo "Example:"
echo "  curl -s -u ${HARBOR_USERNAME}:${HARBOR_PASSWORD} http://${HARBOR_HOST}/v2/${HARBOR_PROJECT}/pause/tags/list"
